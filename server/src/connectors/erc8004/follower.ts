...
async function readObservations(ids: string[], target: BlockRef,
  ...
  const workSignal = AbortSignal.any([signal, child.signal]);
  let failed = false;
  ...
  await Promise.all(Array.from({ length: Math.min(ids.length, 4) }, async () => {
    try {
      while (next < ids.length) {
        ...
        const index = next++;
        const id = ids[index]!;
        const profile = await reader.identity(id, target, workSignal);
        check(workSignal);
...
export async function syncIdentityOnce(config: IdentityFollowerConfig,
  ...
  try {
    ...
    if (confirmed < from) {
      ...
      if (!sameBlock(await reader.block(head.number, signal), head)) {
        check(signal);
        return await withdrawIdentitySource(source, coverage.stateVersion, saved);
      }
      await reader.assertNetwork(signal);
    ...
    for (let reductions = 0; ; reductions++) {
      check(signal);
      try {
        // Bracket every attempted log range, including each reduced retry.
        target = await reader.block(end.toString(), signal);
        check(signal);
